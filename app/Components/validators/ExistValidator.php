...
//        $conditions = [$this->targetAttributeJunction == 'or' ? 'or' : 'and'];
        ...
        if ($this->valueExists($targetClass, $query, $this->_getProperty('attributes')[$attribute])) {
            $this->addError($model, $attribute, $this->message);
        }
    }
    ...
    private function prepareConditions($targetAttribute, $model, $attribute)
    {
        if (is_array($targetAttribute)) {
            ...
            $conditions = [$targetAttribute => $this->_getProperty('attributes')[$attribute]];
        }
    ...
    protected function validateValue($value)
    {
        ...
        var_dump($this->targetAttribute);die;
    ...
    {
        /** @var \App\Model\Model $targetClass */
        $query = $targetClass::query()->where($condition);
        if ($this->filter instanceof \Closure) {
            call_user_func($this->filter, $query);
        } elseif ($this->filter !== null) {
            if (is_array($this->filter) && count($this->filter) == count($this->filter,1)) {
